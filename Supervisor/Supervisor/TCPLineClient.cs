...
{
   // Class for communicating with a TCP server that uses newline as message separator
   class TCPLineClient
   {
      ...
      private byte[] recvBuffer = new byte[256];  // Receive at most 256 bytes in each receive call
      ...
      // Call periodically to receive new messages, send messages in the send queue or connect
      // if not connected.
      public void Update()
      {
         mutex.WaitOne();
      ...
      // Add a message to the send queue. A newline character
      // is appended if the message doesn't already end with one.
      ...
      // Fetch complete received messages
      public string Get()
      {
         mutex.WaitOne();
         if (recvQueue.Count > 0)
